...
if preprocessing:
    ...
    sql = "UPDATE Manholes_Initial_Match INNER JOIN Manholes_GIS_Model_Match ON (Manholes_Initial_Match.FacilityID = Manholes_GIS_Model_Match.FacilityID) AND (Manholes_Initial_Match.Sewer_Area = Manholes_GIS_Model_Match.Sewer_Area) "
    sql += "SET Manholes_GIS_Model_Match.MUID = [Manholes_Initial_Match].[muid], Manholes_GIS_Model_Match.Match_Code = 11 WHERE Acronym_Match = 0 AND Accept_Dist = 1"
    sqls.append(sql)

    sql = "UPDATE Manholes_Initial_Match INNER JOIN Manholes_GIS_Model_Match ON (Manholes_Initial_Match.FacilityID = Manholes_GIS_Model_Match.FacilityID) AND (Manholes_Initial_Match.Sewer_Area = Manholes_GIS_Model_Match.Sewer_Area) "
    sql += "SET Manholes_GIS_Model_Match.MUID = [Manholes_Initial_Match].[muid], Manholes_GIS_Model_Match.Match_Code = 12 WHERE Acronym_Match = 1 AND Accept_Dist = 0"
    sqls.append(sql)

    sqls.append("ALTER TABLE Sewer_Manholes ADD COLUMN Match_Code INTEGER, Pre_Match_MUID TEXT")
    sqls.append("UPDATE Sewer_Manholes INNER JOIN Manholes_GIS_Model_Match ON Sewer_Manholes.FacilityID = Manholes_GIS_Model_Match.FacilityID SET Sewer_Manholes.Match_Code = Manholes_GIS_Model_Match.Match_Code, Sewer_Manholes.Pre_Match_MUID = Manholes_GIS_Model_Match.MUID")
    ...
    #Add field FacilityID
    #Join with match and transfer FacilityID
    #Join new table with match to get all that have not yet been added
    #iterate through these and append
    sqls = []
    sqls.append("UPDATE Mains_GIS_Model_Match SET Map_Match = 0")
##    sqls.append("ALTER TABLE msm_Link ADD COLUMN FacilityID TEXT, Match_Code TEXT")
